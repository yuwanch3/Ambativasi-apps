...
import { saveProgress } from "../../../../../src/utils/progressTracker";
...
const checkApakahBenar = (soal: SoalAI, jawabanUserText: string): boolean => {
  ...
  if (soal.tipe_soal === "fill_blank") {
    ...
    if (Array.isArray(soal.jawaban_benar)) {
      return soal.jawaban_benar.some(
        (opsi) => bersihkanDanLuruskanTeks(opsi) === cleanUser,
      );
    }
...
export default function ReviewJawabanKuis() {
  ...
  const { judul_bab, sumber_data, tipe_sumber } = params;
  ...
  const [exitTarget, setExitTarget] = useState<
    "dashboard" | "profile" | "pengaturan" | "chat" | "shareProgress"
  >("dashboard");
  const fadeExitAnim = useRef(new Animated.Value(0)).current;
  const scaleExitAnim = useRef(new Animated.Value(0.8)).current;

  // 🔒 FUNGSI KEMBALI KE SOAL DENGAN STATUS JAWABAN TERKUNCI (READ-ONLY)
  const kembaliKeSoalLocked = async () => {
    try {
      await AsyncStorage.setItem("isKuisReadOnly", "true");
    } catch (e) {
      console.log("Gagal set readOnly flag", e);
    }
    router.back();
  };
  ...
  }, [judul_bab, sumber_data, tipe_sumber]);
  ...
  const loadReviewData = async () => {
    try {
      ...
      if (dataSoalRaw && jawabanUserRaw) {
        ...
        setSkor(tempSkor);
        const total = parsedSoal.length;
        saveProgress({
          subjectId: String(sumber_data || "unknown").toLowerCase(),
          subjectName: String(judul_bab || ""),
          levelId: "",
          levelName: "",
          babId: String(sumber_data || "unknown").toLowerCase(),
          babName: String(judul_bab || ""),
          type: "latihan",
          score: tempSkor,
          total,
  ...
  // 💡 MODAL PERINGATAN KELUAR UNTUK SEMUA NAVIGASI DARI REVIEW
  const handleBukaModalKeluar = (
    target: "dashboard" | "profile" | "pengaturan" | "chat" | "shareProgress",
  ) => {
    setExitTarget(target);
    setShowExitModal(true);
    Animated.parallel([
      Animated.timing(fadeExitAnim, {
        toValue: 1,
        duration: 350,
        useNativeDriver: true,
      }),
      Animated.spring(scaleExitAnim, {
        toValue: 1,
        friction: 6,
        useNativeDriver: true,
      }),
  ...
  const isCurrentCorrect = checkApakahBenar(
    soalSaatIni,
    jawabanUser[indeksAktif],
  );
  const jawabanSalah = !isCurrentCorrect;

  const isMateriJepang =
    sumber_data?.toString().toLowerCase().includes("nihongo") ||
    sumber_data?.toString().toLowerCase().includes("jepang") ||
    sumber_data?.toString().toLowerCase().includes("bj");

  // RTL HANYA aktif jika materi tersebut Tajwid/Arab
  const wajibRTL = jenisTipe === "full" && !isMateriJepang;
  ...
  const renderTeksPertanyaanReview = (fullText: string = "") => {
    ...
    if (isian && jenisTipe !== "fill_blank" && soalSaatIni?.pilihan) {
      isian =
        soalSaatIni.pilihan[isian as keyof typeof soalSaatIni.pilihan] || isian;
    }
  ...
  return (
    ...
    >
      ...
      <View style={styles.mainContent}>
        {/* 🟢 Tombol Kembali Atas: Kembali ke Soal dengan Status Kuis Terkunci */}
        ...
        >
          <View style={styles.infoSkorUtama}>
            ...
            <Text style={[styles.teksSkor, { color: colors.text }]}>
              {skor}{" "}
              <Text style={[styles.teksTotalSoal, { color: colors.subtext }]}>
                / {listSoal.length}
              </Text>
            </Text>
          </View>
          <View
            style={[styles.pembatasSkor, { backgroundColor: colors.border }]}
          />
          <View style={styles.infoPersentase}>
            ...
            >
              {listSoal.length > 0
                ? Math.round((skor / listSoal.length) * 100)
                : 0}
              %
            </Text>
              ...
              return (
                <SoundTouchableOpacity
                  ...
                  style={[
                    ...
                    indeksAktif !== indeks &&
                      (isCorrect ? styles.angkaBenar : styles.angkaSalah),
                  ]}
        ...
        <View
          style={[
            ...
            { backgroundColor: colors.card, borderColor: colors.border },
            wajibRTL && [
              styles.kartuSoalArab,
              { backgroundColor: colors.isDark ? "#1E293B" : "#F8FAFF" },
            ],
          ]}
        >
          <View style={styles.rowTipeReviewBadge}>
            <Text style={[styles.nomorSoalTitle, { color: colors.subtext }]}>
              {language === "id"
                ? `Pertanyaan ${indeksAktif + 1}`
                : `Question ${indeksAktif + 1}`}
            </Text>
            <View
              style={[
            ...
            >
              <Text style={[styles.txtMiniTipe, { color: colors.subtext }]}>
          ...
          <Text
            style={[
              styles.teksPertanyaan,
              { color: colors.text },
              wajibRTL && styles.teksPertanyaanArab,
            ]}
          >
            ...
            style={[
              ...
              jawabanSalah
                ? [
                    styles.badgeSalah,
                    {
                      backgroundColor: colors.isDark ? "#451A1A" : "#FEF2F2",
                      borderColor: colors.isDark ? "#991B1B" : "#FEE2E2",
                    },
                  ]
                : [
                    styles.badgeBenar,
                    {
                      ...
                      borderColor: colors.isDark ? "#065F46" : "#DCFCE7",
                    },
                  ],
            ]}
          >
        ...
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
          {jenisTipe === "fill_blank" ? (
            <View
              style={[
                styles.boxReviewBlank,
                { backgroundColor: colors.card, borderColor: colors.border },
              ]}
            >
              <Text style={[styles.labelReviewBlank, { color: colors.text }]}>
                {language === "id"
                  ? "Jawaban Input Teks Kamu:"
                  : "Your Input Text Answer:"}
              </Text>
              <View
                style={[
                  styles.inputReviewBlankBox,
                  { backgroundColor: colors.isDark ? "#0F172A" : "#F8FAFC" },
                  isCurrentCorrect ? styles.borderBenar : styles.borderSalah,
                ]}
              >
                <Text
                  style={[
                    styles.textReviewBlankUser,
                    { color: isCurrentCorrect ? "#16A34A" : "#EF4444" },
                  ]}
                >
                  {jawabanUser[indeksAktif] ||
                    (language === "id"
                      ? "(Kosong / Tidak Mengisi)"
                      : "(Blank / Not Answered)")}
                </Text>
                <Ionicons
              ...
              <Text
                style={[
                  styles.labelReviewBlank,
                  { color: colors.text, marginTop: 14 },
                ]}
              >
                {language === "id"
                  ? "Variasi Alternatif Jawaban Yang Sah & Diterima:"
                  : "Valid & Accepted Answer Variants:"}
              </Text>
              <View style={styles.boxAcceptedVariants}>
                {soalSaatIni &&
                  (Array.isArray(soalSaatIni.jawaban_benar) ? (
                    ...
                    >
                      ...
                      >
                        {String(soalSaatIni?.jawaban_benar)}
                      </Text>
                    </View>
                  ))}
              </View>
              ...
              return (
                <View
                  ...
                  style={[
                    ...
                    isCorrectAnswer && [
                      styles.opsiBenarKunci,
                      {
                        backgroundColor: colors.isDark ? "#064E3B" : "#F0FDF4",
                      },
                    ],
                    isWrongUserAnswer && [
                      styles.opsiSalahUser,
                      {
                        backgroundColor: colors.isDark ? "#451A1A" : "#FEF2F2",
                      },
                    ],
        ...
        <View style={styles.navigasiBawah}>
          ...
          {indeksAktif === listSoal.length - 1 ? (
            /* 🟢 Tombol Selesai Review: Kembali ke Soal dengan Status Kuis Terkunci */
            <SoundTouchableOpacity
      ...
      <Sidebar
        ...
        onNavigateChat={() => {
          toggleSidebar(false);
          handleBukaModalKeluar("chat");
        }}
        onNavigateShareProgress={() => {
          toggleSidebar(false);
          handleBukaModalKeluar("shareProgress");
        }}
      />
          ...
          >
            ...
            <SoundTouchableOpacity
              ...
              onPress={() => {
                ...
                } else if (exitTarget === "chat") {
                  router.push("/chat");
                } else if (exitTarget === "shareProgress") {
